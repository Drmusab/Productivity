...
describe('Sync events API', () => {
    ...
    test('captures board creation events', async () => {
        ...
        const createdEvent = eventsResponse.body.find((event) => event.resource === 'board' && event.action === 'created');
    ...
    test('filters events when lastEventId is provided', async () => {
        ...
        expect(filteredEvents.body.some((event) => event.resource === 'board' && event.data.board.name === 'Subsequent Board')).toBe(true);
    });
...
//# sourceMappingURL=sync.events.test.js.map