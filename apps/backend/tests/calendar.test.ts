...
describe('Calendar API', () => {
    ...
    describe('GET /api/calendar/events', () => {
        ...
        it('should return tasks as FullCalendar events', async () => {
            ...
            await runAsync(`INSERT INTO tasks (title, description, column_id, position, priority, due_date, pinned) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`, ['Test Task', 'Test Description', columns[0].id, 0, 'high', '2025-12-30T14:00:00', 1]);
            const response = await request(app)
                .get('/api/calendar/events')
                .expect(200);
            expect(Array.isArray(response.body)).toBe(true);
        ...
        it('should filter tasks by date range', async () => {
            ...
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task 1', columnId, 0, '2025-12-01T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task 2', columnId, 1, '2025-12-15T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task 3', columnId, 2, '2026-01-15T10:00:00']);
            const response = await request(app)
                .get('/api/calendar/events')
        ...
        it('should filter tasks by board', async () => {
            ...
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task Board 1', columns1[0].id, 0, '2025-12-15T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task Board 2', column2.lastID, 0, '2025-12-15T10:00:00']);
            const response = await request(app)
                .get('/api/calendar/events')
        ...
        it('should apply correct priority colors', async () => {
            ...
            await runAsync(`INSERT INTO tasks (title, column_id, position, priority, due_date) VALUES (?, ?, ?, ?, ?)`, ['Low Priority', columnId, 0, 'low', '2025-12-15T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position, priority, due_date) VALUES (?, ?, ?, ?, ?)`, ['Medium Priority', columnId, 1, 'medium', '2025-12-16T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position, priority, due_date) VALUES (?, ?, ?, ?, ?)`, ['High Priority', columnId, 2, 'high', '2025-12-17T10:00:00']);
            const response = await request(app)
                .get('/api/calendar/events')
                .expect(200);
        ...
        it('should not return tasks without due dates', async () => {
            const columns = await allAsync('SELECT id FROM columns');
            const columnId = columns[0].id;
            await runAsync(`INSERT INTO tasks (title, column_id, position, due_date) VALUES (?, ?, ?, ?)`, ['Task with date', columnId, 0, '2025-12-15T10:00:00']);
            await runAsync(`INSERT INTO tasks (title, column_id, position) VALUES (?, ?, ?)`, ['Task without date', columnId, 1]);
            const response = await request(app)
                .get('/api/calendar/events')
                .expect(200);
            expect(response.body.length).toBe(1);
