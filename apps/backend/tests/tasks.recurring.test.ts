...
const createTestTask = async ({ dueDate, recurringRule }) => {
    ...
    const column = await runAsync('INSERT INTO columns (board_id, name, color, icon, position) VALUES (?, ?, ?, ?, ?)', [board.lastID, `QA Column ${timestamp}`, '#ffffff', null, 0]);
    const taskRes = await runAsync(`INSERT INTO tasks (title, description, column_id, position, priority, due_date, recurring_rule, created_by, assigned_to)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
...
describe('createRecurringTask', () => {
    ...
    test('rejects when the original task due date is missing or invalid', async () => {
        ...
        await expect(createRecurringTask(task, recurringRule)).rejects.toThrow('Cannot create recurring task without a valid due date');
        await runAsync('DELETE FROM boards WHERE id = ?', [boardId]);
    });
...
//# sourceMappingURL=tasks.recurring.test.js.map