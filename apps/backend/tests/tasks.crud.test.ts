...
const createBoardWithColumns = async () => {
    ...
    const firstColumn = await runAsync('INSERT INTO columns (board_id, name, color, icon, position) VALUES (?, ?, ?, ?, ?)', [board.lastID, 'Todo', '#e0e0e0', null, 0]);
    const secondColumn = await runAsync('INSERT INTO columns (board_id, name, color, icon, position) VALUES (?, ?, ?, ?, ?)', [board.lastID, 'Doing', '#d0d0d0', null, 1]);
    return {
...
const createTask = async ({ columnId, title = 'Sample Task', position = 0 }) => {
    const result = await runAsync(`INSERT INTO tasks (title, description, column_id, position, priority, pinned)
     VALUES (?, ?, ?, ?, ?, ?)`, [title, 'Description', columnId, position, 'medium', 0]);
    return result.lastID;
};
const createTag = async (name = 'Tag', color = '#000000') => {
    const tag = await runAsync('INSERT INTO tags (name, color) VALUES (?, ?)', [name, color]);
    return tag.lastID;
};
